// RoomSaathi Web Platform Engine - Vercel Ready
const state = {
  currentRole: 'BUYER',
  isLoggedIn: true,
  userProfile: {
  ...
  apkRelease: {
    ...
    releaseNotes: 'Direct web synchronization, instant digital QR check-in tickets, and Gemini AI Concierge search.'
  },
...
function fetchLatestVersionFromApi() {
  fetch('/api/v1/app/latest-version')
    .then(res => res.json())
    .then(data => {
      if (data && data.latest_version) {
        state.apkRelease.version = data.latest_version;
        state.apkRelease.downloadUrl = data.download_url;
        state.apkRelease.fileSize = data.file_size;
        state.apkRelease.releaseDate = data.release_date;
        renderApkSection();
      }
    })
    .catch(() => console.log('Using static fallback APK release configuration'));
}
    ...
    <div class="room-card">
      ...
      <div class="room-body">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 6px;">
          <span style="font-size: 11px; background: #FFF3E0; color: #FF6F00; padding: 2px 8px; border-radius: 12px; font-weight:700;">
            ${p.category}
          </span>
...
function openBookingModal(propId) {
  ...
  if (!prop) return;
  alert(`RoomSaathi Web Checkout:\n\nProperty: ${prop.title}\nPrice: $${prop.price}/night\n\nRedirecting to secure booking payment & QR check-in ticket generation...`);
}
